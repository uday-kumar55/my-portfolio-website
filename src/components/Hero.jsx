  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-4xl mx-auto z-10 flex flex-col items-center justify-center space-y-10 w-full text-center">
        ...
        <div className="space-y-8 flex flex-col items-center text-center w-full">
          ...
          <div className="space-y-6 flex flex-col items-center w-full">
            ...
            >
              Designing <span className="text-gold-600 font-semibold">secure web applications</span> & <span className="text-gold-600 font-semibold">highly interactive</span> user experiences.
            </motion.h2>
            ...
            >
              I engineer high-performance, bulletproof frontend templates. Blending robust React programming with core web security foundations, I establish clean state isolation and accessible user flows.
            </motion.p>
          </div>

          {/* Standard Modern Column Metrics */}
          <div className="grid grid-cols-3 gap-6 pt-8 border-t border-paper-200/60 w-full max-w-xl text-center" id="hero-stats">
            {[
              { value: "03", label: "PROJECTS INSTALLED", note: "DEPLOYED & LIVE" },
              { value: "01", label: "HACKATHON PRIZE", note: "MLRIT APP FIESTA #3" },
              { value: "IIT", label: "NPTEL CLOUD GRAD", note: "CLOUD COMPUTING" }
            ].map((stat, idx) => (
              <motion.div
                key={stat.label}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ type: "spring", stiffness: 100, damping: 14, delay: 0.4 + idx * 0.1 }}
                className="relative overflow-hidden group py-1 text-center flex flex-col items-center"
              >
                <span className="block text-2.5xl font-display font-bold text-ink-950 tracking-tighter">{stat.value}</span>
                <span className="text-[8px] text-ink-950 uppercase tracking-widest block mt-1.5 font-bold leading-none">{stat.label}</span>
                <span className="text-[7.5px] text-ink-500 tracking-wider block mt-1 font-mono font-medium">{stat.note}</span>
              </motion.div>
            ))}
          </div>
      ...
      <div className="absolute bottom-6 left-1/2 -ml-3 cursor-pointer z-10 hidden sm:block">
        ...
        >
          <a href="#about" aria-label="Scroll down to About Section" className="text-ink-500 hover:text-gold-600 transition-colors font-sans">
            <ArrowDown size={14} />
          </a>
        </motion.div>
      </div>
