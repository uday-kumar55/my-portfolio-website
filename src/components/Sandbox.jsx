...
export default function Sandbox() {
  ...
  const securityScore = (referrerStrict ? 35 : 0) + (xssCheck ? 35 : 0) + (jwtHttpOnly ? 30 : 0);
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div className="text-center mb-16 max-w-xl mx-auto flex flex-col items-center">
          ...
          <p className="text-xs sm:text-sm font-sans text-ink-700 font-light leading-relaxed">
            Test real-time client state cycles, switch responsive accents, and toggle security shield presets live in the interface.
          </p>
        </div>
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 bg-paper-200 border-b border-paper-200">
            ...
            <span className="text-[7.5px] font-mono text-gold-700 bg-gold-500/10 border border-gold-600/15 px-2.5 py-0.5 uppercase font-bold tracking-wider rounded-md">STATUS: LIVE</span>
          </div>
          ...
          <div className="p-6 sm:p-10 text-left min-h-[320px] flex flex-col justify-between bg-paper-100">
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'live-react' ? (
                ...
                >
                  <div>
                    <span className="text-[8px] font-mono uppercase text-gold-700 block mb-0.5 font-bold tracking-widest">LOCAL DATA FLUX</span>
                    <h4 className="text-base font-sans font-bold text-ink-950 mb-1.5 uppercase tracking-wide">Interactive Slate Counter</h4>
                    <p className="text-xs text-ink-700 leading-relaxed mb-6 font-light font-sans">
                      Test reactive parameters rendering immediately on click. The parent application layout isolates this atomic hook state from affecting top-level rendering threads.
                    </p>
                    ...
                    <div className="bg-paper-50 p-5 sm:p-6 border border-paper-200 rounded-lg relative overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-6">
                      ...
                      <div className={`absolute top-0 bottom-0 left-0 w-1.5 ${
                        accentId === 'gold' ? 'bg-gold-600' : accentId === 'espresso' ? 'bg-ink-950' : 'bg-gold-500'
                      }`} />
                      
                      <div className="space-y-1 text-left sm:flex-1 pl-3 w-full">
                        <div className="text-[8px] font-mono text-ink-500 uppercase tracking-widest font-bold">REACTIVE REGISTRY</div>
                        <h5 className="text-[11px] font-sans font-semibold text-ink-950 uppercase tracking-wider">LOCAL ACCENT CODENAME</h5>
                        <div className="text-[10px] font-mono text-ink-700 mt-0.5">Sandbox Value: <strong className="text-gold-700 font-bold">{counter}</strong></div>
                      </div>
                  ...
                  <div className="pt-5 border-t border-paper-200/60 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex items-center space-x-3">
                      <span className="text-[8.5px] font-mono text-ink-500 uppercase font-bold tracking-wider">ACCENT STYLE:</span>
                        ...
                        {['gold', 'espresso', 'champagne'].map((color) => (
                          <button
                            ...
                            className={`w-5 h-5 rounded-full border transition-all cursor-pointer ${
                              color === 'gold' ? 'bg-gold-600' : color === 'espresso' ? 'bg-ink-950' : 'bg-gold-500'
                ...
                >
                  <div>
                    <span className="text-[8px] font-mono uppercase text-gold-700 block mb-0.5 font-bold tracking-widest">SECURITY ENVIRONMENT</span>
                    <h4 className="text-base font-sans font-bold text-ink-950 mb-1.5 uppercase tracking-wide">Client Isolation Safeguards</h4>
                    <p className="text-xs text-ink-700 leading-relaxed mb-6 font-light font-sans">
                      Toggle active security shield presets below. Testing how isolation parameters and strict client security protocols block potential data leaks under standard audits.
                    </p>
                    ...
                    <div className="space-y-2.5">
                      ...
                      <div className="flex items-center justify-between p-3.5 hover:bg-gold-500/5 transition-colors border border-paper-200 rounded-lg">
                        <div className="text-left">
                          <h5 className="text-[10px] font-mono font-bold text-ink-950 uppercase tracking-wider">Referrer-Policy: 'no-referrer'</h5>
                          <p className="text-[8px] text-ink-500 mt-0.5 font-sans leading-none">Omit original request headers on external navigations</p>
                        </div>
                        <button
                        ...
                        >
                          <div className={`w-4 h-4 rounded-full bg-paper-100 shadow-sm transition-transform ${
                            referrerStrict ? 'translate-x-[16px]' : 'translate-x-0'
                          }`} />
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-3.5 hover:bg-gold-500/5 transition-colors border border-paper-200 rounded-lg">
                        <div className="text-left">
                          <h5 className="text-[10px] font-mono font-bold text-ink-950 uppercase tracking-wider">XSS Script Block Rule</h5>
                          <p className="text-[8px] text-ink-500 mt-0.5 font-sans leading-none">Filters client components markup against injected script strings</p>
                        </div>
                        <button
                      ...
                      <div className="flex items-center justify-between p-3.5 hover:bg-gold-500/5 transition-colors border border-paper-200 rounded-lg">
                        <div className="text-left">
                          <h5 className="text-[10px] font-mono font-bold text-ink-950 uppercase tracking-wider">Cookie HttpOnly Parameter</h5>
                          <p className="text-[8px] text-ink-500 mt-0.5 font-sans leading-none">Prevents JavaScript processes from extracting session credentials</p>
                        </div>
                        <button
                        ...
                        >
                          <div className={`w-4 h-4 rounded-full bg-paper-100 shadow-sm transition-transform ${
                            jwtHttpOnly ? 'translate-x-[16px]' : 'translate-x-0'
                          }`} />
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="pt-5 border-t border-paper-200 flex items-center justify-between">
                    <div className="flex items-center space-x-2 text-[10px]">
                      {securityScore === 100 ? (
                        <Unlock className="text-gold-600 animate-pulse" size={12} />
                      ) : (
                        <Unlock className="text-ink-500" size={12} />
                      )}
                      <span className="font-mono text-ink-950 text-[9px] uppercase tracking-wider font-bold">
                        VULNERABILITY RATIO: <span className={securityScore === 100 ? 'text-gold-600 font-bold' : 'text-amber-600 font-bold'}>{100 - securityScore}% RISK</span>
                      </span>
                    </div>
                    ...
                    <span className="text-[9px] font-mono text-ink-500 uppercase font-bold bg-paper-50 px-2 py-0.5 border border-paper-200 rounded-md">
                      {securityScore === 100 ? '100% AUDIT CLEARED' : 'NEEDS ATTENTION'}
                    </span>
                  </div>
